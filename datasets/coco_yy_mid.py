...
class COCO(data.Dataset):
  ...
  def __getitem__(self, index):
    ...
    common_dist = np.zeros((self.num_classes, self.fmap_size['h'], self.fmap_size['w']), dtype=np.float32)
    ...
    for k, (bbox, label, orignal_obj_bbox) in enumerate(zip(yy_bboxes, yy_labels, bboxes)):
      if flipped:
        bbox[[0, 2]] = width - bbox[[2, 0]] - 1
      bbox[:2] = affine_transform(bbox[:2], trans_fmap)
      bbox[2:] = affine_transform(bbox[2:], trans_fmap)
      ...
      if h > 0 and w > 0:
        if flipped:
          orignal_obj_bbox[[0, 2]] = width - orignal_obj_bbox[[2, 0]] - 1
        orignal_obj_bbox[:2] = affine_transform(orignal_obj_bbox[:2], trans_fmap)
        orignal_obj_bbox[2:] = affine_transform(orignal_obj_bbox[2:], trans_fmap)
        orignal_obj_bbox[[0, 2]] = np.clip(orignal_obj_bbox[[0, 2]], 0, self.fmap_size['w'] - 1)
        orignal_obj_bbox[[1, 3]] = np.clip(orignal_obj_bbox[[1, 3]], 0, self.fmap_size['h'] - 1)
        h_, w_ = orignal_obj_bbox[3] - orignal_obj_bbox[1], orignal_obj_bbox[2] - orignal_obj_bbox[0]

        orignal_obj_c = np.array([(orignal_obj_bbox[0] + orignal_obj_bbox[2]) / 2,
                                  (orignal_obj_bbox[1] + orignal_obj_bbox[3]) / 2], dtype=np.float32)
        orignal_obj_c_int = orignal_obj_c.astype(np.int32)

        obj_c = np.array([(bbox[0] + bbox[2]) / 2, (bbox[1] + bbox[3]) / 2], dtype=np.float32)
        obj_c_int = obj_c.astype(np.int32)

        # 阴影个数与物体个数对应故在创建yy_heatmap时创建common_dis
        common_c = (orignal_obj_c_int + obj_c_int)/2
        common_c_int = common_c.astype(np.int32)
        common_radius = max(0, int(gaussian_radius((math.ceil(h*h_), math.ceil(w*w_)), self.gaussian_iou)))
        draw_umich_gaussian(common_dist[label-8], common_c_int, common_radius)
    ...
    return {'image': img,
            ...
            'yy_hmap':yy_hmap, 'yy_w_h_':yy_w_h_, 'yy_regs':yy_regs, 'yy_inds':yy_inds, 'yy_ind_masks':yy_ind_masks,
            'common_dis':common_dist}
