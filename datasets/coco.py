...
class COCO(data.Dataset):
  def __init__(self, data_dir, split, split_ratio=1, img_size=512): # split ['train', 'val']
    ...
    self.img_dir = os.path.join(self.data_dir, '%s2017_break' % split)  # 现在在跑去除阴影的实验，_beark需要去掉
    if split in ('test', 'val', 'eval'):
      self.annot_path = os.path.join(self.data_dir, 'annotations', 'image_info_test-dev2017.json')
    else:
      self.annot_path = os.path.join(self.data_dir, 'annotations', 'pascal_train2020.json')
